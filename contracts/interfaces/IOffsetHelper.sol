...
interface IOffsetHelper {
    ...
    function contractRegistryAddress() external view returns (address);

    function deleteEligibleTokenAddress(string memory _tokenSymbol) external;

    function deposit(address _erc20Addr, uint256 _amount) external;

    function eligibleTokenAddresses(string memory)
        external
        view
        returns (address);
    ...
    function sushiRouterAddress() external view returns (address);
    ...
    function withdraw(address _erc20Addr, uint256 _amount) external;
