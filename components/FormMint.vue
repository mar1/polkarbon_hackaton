...
<template>
    ...
    id="modal2">
      ...
      <div class="step w-64 mx-auto bg-white shadow w-50 rounded-lg divide-y divide-gray-200 w-96 my-auto relative top-1 align-middle">
        <div v-bind:class="{ flex: this.isConnected}" class="px-5 py-7">
          <label class="font-semibold text-sm text-gray-600 pb-1 block">Choose your wallet:</label>
              ...
              <span class="inline-block mr-2">Connect</span>
          </button>
        </div>
  ...
  <p class="font-semibold"><input v-model="decarbValue" class="w-8 h-10 px-0 rounded-lg cursor-pointer text-green-500 font-bold" type="text">  CO2 ton<span v-if="decarbValue>1">s 🍃</span><span v-if="decarbValue>10">🍃</span><span v-if="decarbValue>50">🍃</span><span v-if="decarbValue>75">🍃</span><span v-if="decarbValue==100">🍃</span></p>
  </div>
  <br>
  ...
  <input v-model="prices.swapOutExact" type="text" id="swapOut" class="flex items-center justify-between px-3 py-2 bg-white w-full border rounded-lg font-semibold" placeholder="0" disabled>
                ...
                <button @click="formStep(3)" type="button" class="transition duration-200 bg-green-500 hover:bg-green-600 focus:bg-green-700 focus:shadow-sm focus:ring-4 focus:ring-green-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block mt-6">
              <span class="inline-block mr-2">Review Swap</span>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4 inline-block">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
              </svg>
          </button>
          <label class="font-bold text-sm text-gray-600 py-3 block mx-auto text-center">pKarb Price:</label>
          <div class="grid grid-cols-2 gap-4 pt-2 mx-auto text-center">
            <div>
              <p>{{parseFloat(prices.swapRatioGlmr).toFixed(2)}} GLMR</p>
            </div>
            <div>
            <p>{{prices.bct}} $</p></div>
            </div>
  ...
  <div class="step hidden w-64 mx-auto bg-white shadow w-50 rounded-lg divide-y divide-gray-200 w-96 my-auto align-middle">
            <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
        <div class="px-5 py-7">
   
   <p class=" mx-auto text-center py-2"><label class="text-green-500 text-xl font-extrabold text-transparent">Swap Details</label></p>
  <input v-model="prices.swapInExact" type="text" id="swapIn" class="flex items-center justify-between px-3 py-2 bg-white w-full border rounded-lg font-semibold" placeholder="0" disabled>
    ...
    <div>
     ...
     <p class="">{{(prices.bct*decarbValue).toFixed(2)}} $</p></div>
        ...
        <button @click="executeSwap()" type="button" class="transition duration-200 bg-green-500 hover:bg-green-600 focus:bg-green-700 focus:shadow-sm focus:ring-4 focus:ring-green-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block mt-6">
          <span class="inline-block mr-2" id="mintBtn">Mint pKarb</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" id="arrow" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4 inline-block">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
            </svg>
        </button>
      <div v-if="this.selectedChainOption.chainId != 1284">
        <label class="font-bold text-sm text-gray-600 py-3 block mx-auto text-center">Cross-Chain Fees:</label> 
        <p class="mx-auto text-center">{{this.fees}} {{this.selectedChainOption.symbol}}</p>
        </div>
      </div>
        <div class="p-5">
            <div class="grid grid-cols-3 gap-1">
         ...
         swapValue: 0,
          ...
          watch : {
                 decarbValue:function() {
                    let number = parseFloat(this.prices.swapRatioToken * this.decarbValue).toFixed(4)
                    let number2 = parseFloat(this.prices.swapRatioGlmr*this.decarbValue).toFixed(4)
                    this.prices.swapInExact = `${number} ${this.selectedTokenOption.symbol}`
                    this.prices.swapOutExact = `${number2} pKarb`
  ...
  async mounted() {
    ...
    methods: {
      ...
      if (id == 1) {
        this.selectedChainOption.chainId = 1284
      ...
      async getSwapInfo() {
        try {
          ...
          await console.log(swapInfos,'les swap ici')
          this.prices = swapInfos
          this.decarbValue = this.decarbValue
        } catch (err) {
          console.log(err)
        }
      },
      ...
      async getSwapData() {
        try {
          ...
          const response = await this.$axios.get(`https://polkarbon.herokuapp.com/api/getSwap?fromChain=${this.selectedChainOption.chainId}&fromToken=${this.selectedTokenOption.address}&fromAmount=${amount}&fromAddy=${window.address}`)
          let swapInfos = await response.data
  
          let data = await swapInfos.transactionRequest.data
          let value = await swapInfos.params.fromAmount
          ...
          this.swapArgs = decodedArgs
  
          let totalFees = parseFloat(swapInfos.estimate.feeCosts[0].amount)+parseFloat(swapInfos.estimate.gasCosts[0].amount)
          //console.log('Gas Fees: ',(totalFees/1000000000000000000).toFixed(2),this.selectedChainOption.symbol)
          this.fees = (totalFees/1000000000000000000).toFixed(2)
        } catch (err) {
          console.log(err)
        }
      },
      ...
      async executeSwap() {
        console.log('validate swap')
        console.log(this.selectedChainOption.chainId)
        console.log(this.selectedTokenOption.address)
        console.log(this.swapValue)
        console.log(this.swapArgs)
  
        const provider = new ethers.providers.Web3Provider(window.chosenProvider)
  
        if (this.selectedChainOption.chainId == 1284) {
          //native minting handling
            if (this.selectedTokenOption.address == "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE") {
              ...
              console.log('native mint from GLMR')
              await this.doMint(provider,this.swapValue,"0x0000000000000000000000000000000000000000",window.address,"0")
            }
            else {
              //native mint with ERC20, need to approve before swapping
              ...
              let allowance = await this.checkNativeAllowance(provider,window.address,this.selectedTokenOption.address,this.swapValue)
                if (allowance == true) {
                    await this.doMint(provider,this.swapValue,this.selectedTokenOption.address,window.address,"0")
                }
                else {
        ...
        console.log('cross chain mint from', this.selectedChainOption.chainId, this.selectedTokenOption.address, this.swapValue)
          
      }
  
      },
  
  async doMint(provider,amount,token,recipient,collator) {
    const signer = provider.getSigner()
  ...
  let allowance = await tokenContract.allowance(wallet,moonbeamContractAddress)
  ...
  async approveNative(provider,wallet,token,amount) {
    ...
    let approval = await tokenContract.approve(moonbeamContractAddress,amount)
    ...
    document.getElementById('mintBtn').innerText = 'Minting pKarb...'
      ...
      setOption(option,id) {
        ...
        console.log(option.chainId,'to go')
        this.isOptionsExpanded = false;
        }
        else if (id==2) {
        this.selectedTokenOption = option;
        console.log(option,'optio 2')
        console.log(option.chainId,'to go 2')
        this.isOptionsExpanded = false;
        }
        else if (id==3) {
        this.selectedTokenOption = option;
