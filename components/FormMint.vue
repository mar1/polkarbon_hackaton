  ...
  methods: {
    ...
    async getSwapData() {
      try {
        ...
        const response = await this.$axios.get(`/api/getSwap?fromChain=${this.selectedChainOption.chainId}&fromToken=${this.selectedTokenOption.address}&fromAmount=${amount}&fromAddy=${window.address}`)
        let swapInfos = await response.data
